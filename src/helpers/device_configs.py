...
async def create_config_cache_db(
    ...
    validate_point_addresses(config.poll_start_index, config.points)
    ...
    if min_register_number != config.poll_start_index or poll_count != config.poll_count:
        logger.warning(
    ...
    created_config = await create_config_for_device(site_id, device_id, config)
