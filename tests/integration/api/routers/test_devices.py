...
from integration.factories import create_device, create_site, device_payload
...
class TestCreateDevice:
    async def test_create_returns_201_with_defaults(self, client):
        ...
        response = await client.post(devices_url(site["site_id"]), json=device_payload())
        ...
        assert device["points"] == {"standardized": [], "native": [], "virtual": []}
    ...
    async def test_device_type_with_templates_gets_standardized_points(self, client):
        ...
        fetched = await client.get(f"{devices_url(site['site_id'])}/{device['device_id']}")
        standardized = fetched.json()["points"]["standardized"]
        assert standardized
    ...
    async def test_unknown_site_is_404(self, client):
        response = await client.post(devices_url(9999), json=device_payload())
    ...
    async def test_invalid_type_is_422(self, client):
        ...
        response = await client.post(
            devices_url(site["site_id"]), json=device_payload(type="SOLAR")
        )
...
class TestReadDevices:
    async def test_list_is_scoped_to_the_site(self, client):
        ...
        assert [device["name"] for device in response.json()] == ["a-meter"]
    ...
    async def test_device_from_another_site_is_404(self, client):
        ...
        response = await client.get(
            f"{devices_url(second_site['site_id'])}/{device['device_id']}"
        )
        assert response.status_code == 404
...
class TestUpdateDevice:
    async def test_partial_update(self, client):
        ...
        response = await client.put(
            ...
            json={"port": 1502, "poll_enabled": False},
        )
        assert response.status_code == 200
    ...
    async def test_update_unknown_device_is_404(self, client):
        site = await create_site(client)
        response = await client.put(f"{devices_url(site['site_id'])}/9999", json={"port": 1502})
        assert response.status_code == 404
...
class TestDeleteAndRestoreDevice:
    ...
    async def test_restore_active_device_is_409(self, client):
        ...
        device = await create_device(client, site["site_id"])
        response = await client.post(
            f"{devices_url(site['site_id'])}/{device['device_id']}/restore"
        )
        assert response.status_code == 409
